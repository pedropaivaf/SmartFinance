  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-2 px-3 py-2.5 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-[#2D6A4F] dark:text-[#52B788] flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        </svg>
        <p className="text-xs text-[#2D6A4F] dark:text-[#52B788]">{t('settings.openfinance.securityNote')}</p>
      </div>

      {/* Error */}
      {error && (
        <div className="px-3 py-2.5 bg-red-50 dark:bg-red-950/30 rounded-xl">
          <p className="text-xs text-[#9B2226] dark:text-[#E76F51]">{error}</p>
        </div>
      )}
      ...
      {connectedBanks.length > 0 && (
        <div className="space-y-2">
          {connectedBanks.map((bank) => (
            <div key={bank.itemId} className="flex items-center justify-between px-4 py-3 bg-white dark:bg-[#1E1D1C] rounded-2xl border border-[#E8E5E0] dark:border-[#2D2B28] shadow-sm">
              <div className="flex items-center gap-3">
                {bank.logo
                  ? <img src={bank.logo} alt={bank.name} className="w-8 h-8 rounded-full object-contain" />
                  : <span className="w-8 h-8 rounded-full bg-[#F4F3EF] dark:bg-[#2D2B28] flex items-center justify-center text-lg">🏦</span>
                }
                <div>
                  <p className="text-sm font-medium text-[#1A1A1A] dark:text-[#E8E4DF]">{bank.name}</p>
                  <p className="text-xs text-[#2D6A4F] dark:text-[#52B788]">{t('settings.openfinance.connected')}</p>
                </div>
              </div>
              <button type="button" onClick={() => handleDisconnect(bank.itemId)}
                className="text-xs text-[#9B2226] dark:text-[#E76F51] hover:underline">
      ...
      {phase === 'done' && importedCount > 0 && importStats && (
        <div className="px-4 py-3 bg-[#E8F0F4] dark:bg-[#1B2B35] rounded-xl space-y-1.5">
          <p className="text-sm font-semibold text-[#1B4965] dark:text-[#5FA8D3]">
            {t('settings.openfinance.success', { count: importedCount })}
          </p>
          <div className="flex flex-wrap gap-2 text-xs">
            {importStats.incomeCount > 0 && (
              <span className="px-2 py-0.5 rounded-full bg-[#E8F0F4] dark:bg-[#1B2B35] text-[#1B4965] dark:text-[#5FA8D3]">
                {importStats.incomeCount} entradas
              </span>
            )}
            {importStats.expenseCount > 0 && (
              <span className="px-2 py-0.5 rounded-full bg-red-100 dark:bg-red-900/40 text-[#9B2226] dark:text-[#E76F51]">
      ...
      {phase === 'importing' && (
        <div className="flex flex-col items-center gap-3 py-6">
          <svg className="animate-spin h-8 w-8 text-[#1B4965] dark:text-[#5FA8D3]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
          <p className="text-sm text-[#6B6B6B] dark:text-[#A09A92]">Importando transações...</p>
        </div>
      )}
      ...
      {phase === 'done' && grouped && grouped.total > 0 && (
        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-wider text-[#6B6B6B] dark:text-[#A09A92]">
          ...
          {grouped.income.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-medium text-[#1B4965] dark:text-[#5FA8D3] px-1 flex items-center gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M7 11l5-5m0 0l5 5m-5-5v12" />
                </svg>
                Entradas ({grouped.income.length})
              </p>
              <div className="rounded-xl bg-white dark:bg-[#1E1D1C] border border-[#E8E5E0] dark:border-[#2D2B28] shadow-sm divide-y divide-[#E8E5E0]/80 dark:divide-[#2D2B28]/40 max-h-32 overflow-y-auto">
          ...
          {grouped.expenses.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-medium text-[#9B2226] dark:text-[#E76F51] px-1 flex items-center gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M17 13l-5 5m0 0l-5-5m5 5V6" />
                </svg>
                Despesas ({grouped.expenses.length})
              </p>
              <div className="rounded-xl bg-white dark:bg-[#1E1D1C] border border-[#E8E5E0] dark:border-[#2D2B28] shadow-sm divide-y divide-[#E8E5E0]/80 dark:divide-[#2D2B28]/40 max-h-40 overflow-y-auto">
          ...
          <button type="button" onClick={handleImport}
            className="w-full py-3 rounded-2xl bg-[#1B4965] hover:bg-[#153B52] text-white text-sm font-semibold transition-colors">
      ...
      {phase === 'done' && pendingTransactions.length === 0 && importedCount === 0 && (
        <p className="text-sm text-[#6B6B6B] dark:text-[#A09A92] text-center py-2">
      ...
      {(phase === 'idle' || phase === 'loading') && (
        <button type="button" onClick={handleConnect} disabled={phase === 'loading'}
          className={`w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-semibold transition-colors focus:outline-none ${
            phase === 'loading'
              ? 'bg-[#E8E5E0] dark:bg-[#2D2B28] text-[#9B9B9B] cursor-wait'
      ...
      {/* Supported banks */}
      <div>
        <p className="text-xs font-medium text-[#6B6B6B] dark:text-[#A09A92] mb-2">{t('settings.openfinance.banks')}</p>
        <div className="flex flex-wrap gap-2">
          {SUPPORTED_BANKS.map((bank) => (
            <span key={bank.connector} className="text-xs px-2 py-1 rounded-full bg-[#F4F3EF] dark:bg-[#2D2B28] text-[#6B6B6B] dark:text-[#A09A92]">
              {bank.logo} {bank.name}
            </span>
          ))}
        </div>
    ...
    <div className="flex items-center gap-2.5 px-3 py-2.5">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <p className="text-sm font-medium text-[#1A1A1A] dark:text-[#E8E4DF] truncate">{tx.description}</p>
        </div>
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-[10px] text-[#9B9B9B] dark:text-[#6B6560]">{formatDate(tx.createdAt)}</span>
          {tx.category && tx.category !== 'Outros' && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#F4F3EF] dark:bg-[#2D2B28] text-[#6B6B6B] dark:text-[#A09A92]">
              {tx.category}
            </span>
          )}
          {tx.paymentMethod === 'pix' && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/40 text-[#2D6A4F] dark:text-[#52B788]">Pix</span>
          )}
        </div>
      </div>
      <p className={`text-sm font-semibold flex-shrink-0 ${
        tx.type === 'income' ? 'text-[#1B4965] dark:text-[#5FA8D3]' : 'text-[#9B2226] dark:text-[#E76F51]'
      }`}>
